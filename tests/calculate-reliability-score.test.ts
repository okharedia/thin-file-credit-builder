...
import {
  ...
  calculateLateFeePoints,
  calculateNegativeBalancePoints,
...
test("rejects invalid income regularity metrics", () => {
  assert.throws(
    () =>
      calculateIncomeRegularityPoints({
        incomeMonthCount: 7,
  ...
  assert.throws(
    () =>
      calculateIncomeRegularityPoints({
        incomeMonthCount: 0,
        scoringWindowMonthCount: 0,
      }),
    new RangeError("scoringWindowMonthCount must be a positive integer"),
  );
});
...
test("calculates late fee points", () => {
  ...
  for (const { lateFeeEventCount, expectedPoints } of cases) {
    assert.equal(
      calculateLateFeePoints({ lateFeeEventCount }),
      expectedPoints,
    );
  }
});
