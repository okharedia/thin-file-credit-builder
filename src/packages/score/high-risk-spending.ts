...
export function calculateHighRiskSpendingPoints({ totalHighRiskDebitCents, totalSpendingDebitCents }: Pick<ReliabilityMetrics, "totalHighRiskDebitCents" | "totalSpendingDebitCents">): number {
    if (!Number.isSafeInteger(totalHighRiskDebitCents) || totalHighRiskDebitCents < 0 || !Number.isSafeInteger(totalSpendingDebitCents) || totalSpendingDebitCents < 0) {
        throw new RangeError("high-risk and spending totals must be non-negative integers");
    }

    if (totalHighRiskDebitCents > totalSpendingDebitCents) {
        throw new RangeError("totalHighRiskDebitCents cannot exceed totalSpendingDebitCents");
    }
