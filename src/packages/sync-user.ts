...
export function mkSyncUser(args: BankingArgs & DatabaseArgs) {
  ...
  return async (userId: string) => {
    const [accounts, dataRange] = await Promise.all([
      listAccounts(userId),
      getDataRange(),
    ]);
    ...
    const newTransactions = pageTotals.reduce(
      (total, page) => total + page.newTransactions,
      0,
    );
    const duplicateTransactions = pageTotals.reduce(
      (total, page) => total + page.duplicateTransactions,
      0,
    );
